...
const Pocket = React.memo(
  ({
    ...
    // Enhanced home pile press handler
    const handleHomePilePress = useCallback(
      pieceId => {
        console.log(`Pocket: Home piece ${pieceId} pressed`);
        ...
        console.log(`Pocket: Selecting home piece ${pieceId}`);
    ...
    // Enhanced selection highlight logic for home pieces
    const showSelectionHighlight = useCallback(
      pieceId => {
        if (selectedToken === pieceId) {
          return true;
        }
        ...
        if (
          ...
          !selectedToken &&
          isCurrentTurn &&
          diceValue === 6 && // Need 6 to move out
          !isGamePaused
        ) {
          return true;
        }
      ...
      [selectedToken, homePieces, isCurrentTurn, diceValue, isGamePaused],
    );
